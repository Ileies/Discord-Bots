...
export default {
	...
	async execute(interaction, storage) {
		...
		await reply(interaction, embed().setTitle("Commands").addFields(
			...
			{name:"Help & Support",value:"If you encounter a problem or find a bug, feel free to contact me: " + storage.get("owner").tag, inline:true}
		).setFooter({text: "© 2023 - Made with ❤️ by " + storage.get("owner").tag}));
	}
} as Command;